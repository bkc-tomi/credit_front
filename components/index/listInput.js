...
export default function ListInput({ inputs, propsValue=[], func=null }) {
    ...
    const handleSave = () => {
        const val = {};
        inputs.map(input => {
            var elm = document.getElementById(`${ input.id }_input`);
            val[input.id] = elm.value;
        });
        if (func) {
            func([...propsValue, val]);
        }
        ...
        <div className="pb-4">
            ...
            <div className="grid grid-cols-5" style={show}>
                <div className="col-span-4">
                    {
                        inputs.map((input, index) => {
                            switch (input.type) {
                                case "mail":
                                    return (
                                        <div className="md:grid md:grid-cols-6 m-1" key={ index }>
                ...
                <div className="flex justify-center items-center">
                    <Button cls="px-4 py2 md:px-8 md:py-3" func={ () => handleSave() } disabled={ false }>
                        <svg width="1.5em" height="1.5em" viewBox="0 0 16 16" className="bi bi-check2-circle" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
